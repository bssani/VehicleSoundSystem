...
void UExhaustSoundLayer::Initialize(UVehicleSoundComponent* InOwner, UDynamicSoundDataAsset* InDataAsset)
{
	...
	if (DataAsset)
	{
		AudioComponent = CreateAudioComponent(DataAsset->ExhaustConfig.MetaSoundSource, DataAsset->ExhaustConfig.AttenuationOverride, DataAsset->ExhaustConfig.ConcurrencyOverride);
	}
}
